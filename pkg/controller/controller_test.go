...
func TestNewController(t *testing.T) {
	...
	myNs := myNamespace()
	prefix, _ := initKeyPrefix("hello")
	var tweakopts func(*metav1.ListOptions) = nil
	// v2 := schema.GroupVersion{Group: "my-group", Version: "v2"}
	// d := serializer.NewCodecFactory(clientsetscheme.Scheme).UniversalDecoder(v2)
	ctx := context.Background()
	//obj, _, _ := d.Decode([]byte(yml), nil, nil)
	ss_client_set, _ := sealedsecrets.NewForConfig(nil)
	client := fake.NewSimpleClientset()
	keyRegistry, _ := initKeyRegistry(ctx, ss_client_set, rand.Reader, myNs, prefix, SealedSecretsKeyLabel, 2048)
	sinformer := informers.NewFilteredSharedInformerFactory(ss_client_set, 0, namespace, tweakopts)
	ssinformer := ssinformers.NewFilteredSharedInformerFactory(ss_client_set, 0, namespace, tweakopts)
	NewController(client, ss_client_set, ssinformer, sinformer, keyRegistry)
}
